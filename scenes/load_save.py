...
⚠ it's recommended to delete those save file when you imported them
        ...
        self.save_file_canvas = tk.Canvas(
            app,
        ...
        self.save_file_lbl = ttk.Label(
            self.save_file_canvas, text="import a .cpsave file"
        )
        self.save_file_btn = ttk.Button(
            self.save_file_canvas,
            text="🗁",
            command=lambda: self.import_save_file(),
            style="Main.TButton",
        )
        ...
        self.key_file_canvas = tk.Canvas(
            app,
            bg=style.Color.DARKEST,
            borderwidth=0,
            relief="flat",
        ...
        self.key_file_btn = ttk.Button(
            self.key_file_canvas,
            text="🗁",
            command=lambda: self.import_key_file(),
            style="Main.TButton",
        )
        ...
        self.master_key_canvas = tk.Canvas(
            app,
            bg=style.Color.DARKEST,
            borderwidth=0,
            relief="flat",
        ...
        self.load_btn = ttk.Button(
            self.nav_btns, text="OK", command=lambda: self.load(), style="Main.TButton"
        )
        self.return_btn = ttk.Button(
            self.nav_btns,
            text="⤶",
            command=lambda: [self.hide(), loopback()],
            style="Main.TButton",
        )
    ...
    def import_save_file(self):
        filetypes = {("All files", "*.*"), ("Cool Password Save files", ".cpsave")}
    ...
    def import_key_file(self):
        filetypes = {("All files", "*.*"), ("Cool Password Key files", ".cpkey")}
